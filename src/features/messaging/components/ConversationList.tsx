...
import { type ConversationListRow } from '../actions/conversation-actions';
...
export function ConversationList({ initialConversations }: ConversationListProps) {
  ...
  // For V1, we'll just use the initial conversations. 
  // Realtime updates will be added in the store step later.
  const filteredConversations = useMemo(() => {
    ...
    return initialConversations.filter((conv) => {
      ...
      const matchMessage = conv.latestMessage?.content?.toLowerCase().includes(query);
      ...
      <div className="flex flex-col flex-1">
        {initialConversations.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-slate-500">
            ...
            <p className="text-xs mt-2">Once you contact a host or receive a booking enquiry, your conversations will appear here.</p>
          </div>
          ...
          filteredConversations.map((conv) => {
            ...
            const initials = conv.otherParticipant.name.substring(0, 2).toUpperCase();

          // Resolve avatar URL
          let avatarUrl = conv.otherParticipant.avatar_url;
          ...
          const timeString = conv.latestMessage 
            ? formatDistanceToNowStrict(new Date(conv.latestMessage.created_at), { addSuffix: false })
            : formatDistanceToNowStrict(new Date(conv.updated_at), { addSuffix: false });
          ...
          return (
            ...
            >
              <Avatar className="h-12 w-12 shrink-0">
                <AvatarImage src={avatarUrl || undefined} alt={conv.otherParticipant.name} />
              ...
              <div className="flex-1 min-w-0 flex flex-col">
                ...
                <div className="flex items-center justify-between gap-2">
                  <p className={cn(
                    "text-sm truncate",
                    displayUnreadCount > 0 ? "text-slate-900 font-medium" : "text-slate-500"
                  )}>
