...
export function FeaturesForm({ listingId, initialData, amenities }: FeaturesFormProps) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <Select 
            ...
            onValueChange={(val: any) => setValue('occupancy_type', val, { shouldValidate: true })}
          >
          ...
          {errors.occupancy_type && <p className="text-sm text-red-500">{errors.occupancy_type.message}</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input 
            ...
            className="h-12"
          />
          {errors.max_occupants && <p className="text-sm text-red-500">{errors.max_occupants.message}</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          <Select 
            ...
            onValueChange={(val: any) => setValue('furnishing', val, { shouldValidate: true })}
          >
            <SelectTrigger className="h-12">
          ...
          {errors.furnishing && <p className="text-sm text-red-500">{errors.furnishing.message}</p>}
        </div>

        <div className="space-y-2">
          <Label htmlFor="gender_preference">Gender Preference</Label>
          <Select 
            value={watch('gender_preference')} 
            onValueChange={(val: any) => setValue('gender_preference', val, { shouldValidate: true })}
          >
            <SelectTrigger className="h-12">
          ...
          {errors.gender_preference && <p className="text-sm text-red-500">{errors.gender_preference.message}</p>}
        </div>
      </div>
      ...
      <div className="pt-6 border-t border-slate-100">
        ...
        <div className="space-y-8">
          {Object.entries(amenitiesByCategory).map(([category, items]) => (
            <div key={category}>
              <h4 className="font-medium text-slate-700 capitalize mb-3">{category}</h4>
                    ...
                    render={({ field }) => {
                      ...
                      return (
                        <div className="flex items-center space-x-3">
                          ...
                          />
                          <Label htmlFor={amenity.id} className="font-normal text-slate-600 cursor-pointer">
                            {amenity.name}
                          </Label>
                        </div>
                      );
