...
export function NotificationRow({ notification, onMarkRead }: NotificationItemProps) {
  ...
  const timeString = formatDistanceToNowStrict(new Date(notification.created_at), { addSuffix: true });

  const Content = (
    <div 
      className={cn(
        "group flex items-start gap-4 p-4 rounded-xl transition-all duration-200 border",
        isRead ? "bg-white border-transparent hover:border-slate-200 hover:bg-slate-50" : "bg-blue-50/40 border-blue-100 hover:bg-blue-50/60"
      )}
      onClick={() => {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-1">
          <h4 className={cn("text-sm font-semibold truncate", isRead ? "text-slate-700" : "text-slate-900")}>
            {notification.title}
          </h4>
          <span className={cn("text-xs whitespace-nowrap", isRead ? "text-slate-400" : "text-blue-600 font-medium")}>
            {timeString}
          </span>
        </div>
        
        <p className={cn("text-sm leading-snug line-clamp-2", isRead ? "text-slate-500" : "text-slate-700")}>
          {notification.message}
        </p>

        {notification.link && (
          <div className="mt-3 flex items-center text-sm font-medium text-slate-900 group-hover:text-blue-600 transition-colors">
            {getLinkText(notification.type)} <ChevronRight className="w-4 h-4 ml-1" />
          </div>
        )}
      </div>
