...
export function StayList({
  ...
  return (
    <div className="space-y-6">
      {stays.map((stay) => {
        const otherUser =
          viewType === 'host' ? stay.guest : stay.listings?.host;
        const otherUserName =
          otherUser?.full_name ||
          (viewType === 'host' ? 'Anonymous Guest' : 'Host');
                ...
                <div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-x-8 gap-y-2 text-sm text-gray-500">
                    <div>
                      ...
                      {format(
                        new Date(stay.expected_move_in_date),
                        'MMM d, yyyy'
                      )}
                    </div>
                    <div>
                      ...
                      {format(
                        new Date(stay.expected_move_out_date),
                        'MMM d, yyyy'
                      )}
                    </div>
                    {stay.actual_move_in_date && (
                      <div>
                        ...
                        {format(
                          new Date(stay.actual_move_in_date),
                          'MMM d, yyyy'
                        )}
                      </div>
                    )}
                    {stay.actual_move_out_date && (
                      <div>
                        ...
                        {format(
                          new Date(stay.actual_move_out_date),
                          'MMM d, yyyy'
                        )}
                      </div>
                    )}
                  ...
                  {viewType === 'guest' &&
                    ...
                    (!stay.reviews || stay.reviews.length === 0) && (
                      <Button
                        ...
                        onClick={() =>
                          setReviewingStayId(
                            stay.id === reviewingStayId ? null : stay.id
                          )
                        }
                        className="w-full"
                      >
