...
export function MarkerLayer({ listings }: MarkerLayerProps) {
  ...
  const mapListings = listings.filter(l => l.location.latitude != null && l.location.longitude != null);
    ...
    <>
      {mapListings.map(listing => (
        ...
        >
          ...
          >
            <div className={`bg-white rounded-full px-3 py-1 shadow-md border font-semibold text-sm transition-colors ${hoveredListingId === listing.publicId ? 'bg-gray-900 text-white' : 'text-gray-900 hover:bg-gray-900 hover:text-white'}`}>
