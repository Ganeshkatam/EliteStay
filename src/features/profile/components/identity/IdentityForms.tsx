...
function InlineForm({
  ...
  onSubmitData: (data: any) => Promise<{ error?: { message: string } } | { data: any }>;
  close?: () => void;
  children: (props: { register: any; errors: any; isPending: boolean; watch: any; setValue: any }) => React.ReactNode;
}) {
  const { register, watch, setValue, getValues, trigger, formState: { errors, isDirty } } = useForm({
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center pt-2">
        <div className="text-sm font-medium h-5 flex items-center">
          {isPending && <><Loader2 className="mr-2 h-4 w-4 animate-spin text-slate-500" /><span className="text-slate-500">Saving...</span></>}
          {!isPending && isDirty && !error && <span className="text-green-600">Changes saved</span>}
        </div>
        <Button type="button" className="bg-slate-900 text-white hover:bg-slate-800" onClick={() => { close?.(); context?.close(); }}>
          Done
        </Button>
      </div>
    </div>
...
export function NameForm({ profile, close }: { profile: ExtendedProfile, close?: () => void }) {
  return (
    <InlineForm
      schema={z.object({ full_name: z.string().min(2, 'Name must be at least 2 characters') })}
      defaultValues={{ full_name: profile.full_name || '' }}
      onSubmitData={(data) => updateProfile(data as any)}
      close={close}
    >
      {({ register, errors }) => (
        <div>
          ...
          <p className="text-xs text-slate-500 mt-2">This is the name on your travel document, which could be a license or a passport.</p>
        </div>
      )}
    </InlineForm>
  );
...
export function DateOfBirthForm({ profile, close }: { profile: ExtendedProfile, close?: () => void }) {
  return (
    <InlineForm
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar
                  ...
                  selected={date}
                  onSelect={(newDate) => {
                    setValue('date_of_birth', newDate ? format(newDate, 'yyyy-MM-dd') : null, { shouldDirty: true });
                  }}
                  defaultMonth={date}
...
export function GenderForm({ profile, close }: { profile: ExtendedProfile, close?: () => void }) {
  return (
    <InlineForm
...
export function OccupationForm({ profile, close }: { profile: ExtendedProfile, close?: () => void }) {
  return (
    <InlineForm
...
export function BioForm({ profile, close }: { profile: ExtendedProfile, close?: () => void }) {
  return (
    <InlineForm
      schema={z.object({ bio: z.string().max(500, 'Bio must be less than 500 characters').optional().nullable() })}
...
export function UsernameForm({ profile, close }: { profile: ExtendedProfile, close?: () => void }) {
  return (
    <InlineForm
      schema={z.object({
        username: z.string()
      ...
      {({ register, errors }) => (
        <div>
          ...
          <div className="flex">
            ...
            <Input {...register('username')} placeholder="john_smith" className="rounded-l-none" />
          </div>
...
export function TimezoneForm({ profile, close }: { profile: ExtendedProfile, close?: () => void }) {
...
export function PhoneForm({ profile, close }: { profile: ExtendedProfile, close?: () => void }) {
  return (
    <InlineForm
      schema={z.object({ phone: z.string().regex(/^\+91\d{10}$/, 'Phone must be in format +91XXXXXXXXXX').optional().nullable().or(z.literal('')) })}
