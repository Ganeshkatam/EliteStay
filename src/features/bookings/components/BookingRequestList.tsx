...
export function BookingRequestList({ initialBookings }: { initialBookings: Booking[] }) {
  ...
  const handleResponse = async (bookingId: string, action: 'approve' | 'reject') => {
  ...
  return (
    <div className="space-y-6">
      {bookings.map((booking) => (
        <div key={booking.id} className="overflow-hidden rounded-xl bg-white shadow border border-gray-200">
          <div className="p-6 sm:flex sm:items-start sm:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h4 className="text-lg font-bold text-gray-900">{booking.guest?.full_name || 'Anonymous Guest'}</h4>
                <p className="text-sm text-gray-500 mt-1">Requested to book <span className="font-medium text-gray-900">{booking.listings?.title}</span></p>
                <div className="mt-2 flex items-center gap-4 text-sm text-gray-500">
                  <div className="flex items-center gap-1.5">
                    ...
                    Move in: {format(new Date(booking.requested_move_in), 'MMM d, yyyy')}
                  </div>
                  <div className="flex items-center gap-1.5">
                    <svg className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
                ...
                <div className="mt-4 flex flex-col gap-2 w-full sm:w-auto">
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="w-1/2"
                    >
                      {loadingId === booking.id && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                      Approve
                    </Button>
                  </div>
                  <Button
                  ...
                  >
                    {messagingId === booking.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <MessageSquare className="mr-2 h-4 w-4" />}
                    Message Guest
                  </Button>
                </div>
              )}
