...
export default async function BuildListingLayout({
  ...
  const percentComplete = listing.listing_build_progress?.[0]?.percent_complete || 0;
    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b bg-white px-6">
        <div className="flex items-center gap-4">
          <Link href="/host/listings" className="text-sm font-medium text-slate-500 hover:text-slate-900">
            Exit
          </Link>
          <div className="h-4 w-px bg-slate-300" />
          <span className="text-sm font-semibold text-slate-900">Build Listing</span>
        </div>
        <div className="flex items-center gap-4">
        ...
        <aside className="hidden w-64 flex-shrink-0 border-r bg-slate-50/50 md:block p-6">
          <nav className="space-y-4">
            {STEPS.map((step, index) => {
              ...
              return (
                <div key={step.id} className="flex items-center gap-3 text-slate-500">
