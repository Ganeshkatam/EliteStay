...
function StatusBadge({ status }: { status: string }) {
  switch (status) {
    ...
    case 'pending_review':
      return <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-100">In Review</Badge>;
    case 'paused':
      return <Badge className="bg-orange-100 text-orange-800 hover:bg-orange-100">Paused</Badge>;
    case 'archived':
...
export default async function HostListingsPage() {
  ...
  const { data: listings } = await supabase
    ...
    .select(`
      ...
      location:listing_locations(city, locality),
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage your properties and inventory.</p>
        </div>
      ...
      <div className="flex items-center gap-2 max-w-sm">
        <div className="relative flex-1">
          ...
          <Input type="search" placeholder="Search listings..." className="pl-9" />
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {listings?.length === 0 ? (
                ...
                listings?.map((listing: any) => {
                  ...
                  const accType = listing.accommodation_type?.name || 'Unknown Type';
                  const loc = listing.location?.[0] ? `${listing.location[0].locality}, ${listing.location[0].city}` : 'No location';
                  ...
                  return (
                    <tr key={listing.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-slate-500 text-xs mt-1">{accType}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-slate-600 max-w-[200px] truncate" title={loc}>
                        {loc}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link href={listing.status === 'published' ? `/host/listings/${listing.id}` : `/host/listings/${listing.id}/build/accommodation`}>
