...
export default async function HostDashboardPage() {
  ...
  // Fetch quick stats for this host
  ...
  const draftCount =
    listings?.filter((l) => l.status === 'draft' || l.status === 'ready')
      .length || 0;

  // Check if there is an active draft with build progress
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Manage your properties, reservations, and earnings.
          </p>
        </div>
          ...
          <CardContent className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-6 gap-4">
            <div>
              <h2 className="text-lg font-semibold text-blue-900">
                Continue building your listing
              </h2>
              <p className="text-sm text-blue-700 mt-1">
                You have a draft for &quot;
                {Array.isArray(recentDraft.listings)
                  ? recentDraft.listings[0]?.title
                  : (recentDraft.listings as { title?: string })?.title ||
                    'Untitled Listing'}
                &quot; that is {recentDraft.percent_complete}% complete.
              </p>
            </div>
            <Link
              href={`/host/listings/${recentDraft.listing_id}/build/${recentDraft.last_step}`}
            >
