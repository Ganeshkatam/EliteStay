...
-- 1. Create a composite index for spatial queries
CREATE INDEX IF NOT EXISTS idx_listings_lat_lng ON public.listings(latitude, longitude);

-- 2. Drop the old function to avoid signature clashes
DROP FUNCTION IF EXISTS public.search_listings(TEXT, TEXT, UUID, public.furnishing, public.gender_preference, public.occupancy_type, public.billing_period, TEXT[], NUMERIC, NUMERIC, DATE, TEXT, INTEGER, INTEGER);

-- 3. Re-create the function with new bounding box and center parameters (DOUBLE PRECISION)
...
-- 4. Apply permissions (needs to match infrastructure_security.sql patterns if relevant)
ALTER FUNCTION public.search_listings(TEXT, TEXT, UUID, public.furnishing, public.gender_preference, public.occupancy_type, public.billing_period, TEXT[], NUMERIC, NUMERIC, DATE, TEXT, INTEGER, INTEGER, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION) SET search_path = public, pg_temp;
REVOKE EXECUTE ON FUNCTION public.search_listings(TEXT, TEXT, UUID, public.furnishing, public.gender_preference, public.occupancy_type, public.billing_period, TEXT[], NUMERIC, NUMERIC, DATE, TEXT, INTEGER, INTEGER, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.search_listings(TEXT, TEXT, UUID, public.furnishing, public.gender_preference, public.occupancy_type, public.billing_period, TEXT[], NUMERIC, NUMERIC, DATE, TEXT, INTEGER, INTEGER, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION) TO anon, authenticated;
